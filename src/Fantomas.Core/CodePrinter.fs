...
and genTypeAndParam astContext (typeName: Context -> Context) (tds: SynTyparDecls option) tcs =
    ...
    | Some (SynTyparDecls.PrefixList (tds, _range)) ->
        types (!- "(") tds [] (!- ")")
        +> !- " "
        +> typeName
    ...
    let expr =
        ...
        | DotGet (e, sli) ->
            ...
            let longExpr =
                ...
                genExpr astContext e
                +> indentSepNlnUnindent (genSynLongIdentMultiline true sli)
                ...
                atCurrentColumn
                    (sepOpenAnonRecd
                     +> col sepNln fields (fun (AnonRecordFieldName (ident, eq, e, range)) ->
                         ...
                         addFixedSpaces targetColumn
                         +> atCurrentColumn (
                             enterNodeFor SynExpr_AnonRecd_Field range
                             +> genIdent ident
                         )
...
and genAlternativeAppWithSingleParenthesisArgument (e, lpr, a, rpr, _pr) astContext =
    ...
    +> sepSpaceWhenOrIndentAndNlnIfExpressionExceedsPageWidth
        ...
        (sepOpenTFor lpr
         +> expressionFitsOnRestOfLine
             (genExpr astContext a)
             (indentSepNlnUnindent (genExpr astContext a)
              +> sepNln)
         +> sepCloseTFor rpr)
...
and genLetBindingDestructedTuple
    ...
    +> (fun ctx ->
        ...
        let long =
            prefix
            +> indentSepNlnUnindent (genExpr astContext e)
...
and genSynBindingValue
    ...
    +> (fun ctx ->
        ...
        let long =
            prefix
            +> autoIndentAndNlnExpressUnlessRagnarok (genExpr astContext) e
